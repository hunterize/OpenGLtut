...
namespace Materials
{
	...
	void GLSLMaterials()
	{
		...
		window = SDL_CreateWindow("Phong Lighting Window - Materials", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, screenWidth, screenHeight, SDL_WINDOW_OPENGL);
		...
		cubeShader.AttachShader("Shaders/MaterialsVertexShader.vert", "Shaders/MaterialsFragmentShader.frag");
		...
		while (isRunning)
		{
			...
			glm::vec3 objColor(0.2f, 1.0f, 1.0f);
			glm::vec3 objSpecular(0.5f, 0.5f, 0.5f);

			glm::vec3 lightColor(1.0f, 1.0f, 1.0f);
			lightColor.x *= float(sin(SDL_GetTicks() / 1000.0f));
			lightColor.y *= float(cos(SDL_GetTicks() / 1000.0f));
			//lightColor.z *= float(sin(SDL_GetTicks() + 150.0f / 1000.0f));

			float lightAmbientStrength = 0.25f;
			float lightDisffuseStrength = 1.0f;
			...
			cubeShader.SetUniformVec3("material.ambient", objColor);	
			cubeShader.SetUniformVec3("material.diffuse", objColor);
