...
void CMesh::Render(const CShader& shader)
{
	...
	//bind texture to shader, texture unit starts with 100
	for (int i = 0; i < m_Textures.size(); i++)
	{
		glActiveTexture(GL_TEXTURE0 + i + 100);
		...
		shader.SetUniformInt(name + number, i + 100);
...
void CMesh::Init()
{
	...
	glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(CVertex), (void*)offsetof(CVertex, m_Tangent));
	
	glEnableVertexAttribArray(4);
	glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, sizeof(CVertex), (void*)offsetof(CVertex, m_Bitangent));
