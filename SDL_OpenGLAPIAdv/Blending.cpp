...
namespace Blending
{
	...
	void Blending()
	{
		...
		window = SDL_CreateWindow("Adv OpenGL - Blending", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, screenWidth, screenHeight, SDL_WINDOW_OPENGL);
		...
		cubeShader.AttachShader("Shaders/BlendingTestingVertexShader.vert", "Shaders/BlendingTestingFragmentShader.frag");
		...
		grassTexture = CSTexture::LoadImage("grass.png");
		...
		while (isRunning)
		{
			...
			//draw grass
			for (int i = 0; i < sizeof(grass_pos) / sizeof(glm::vec3); i++)
			{
				glm::mat4 model;
