...
void SDLCoordinate3DMultiple()
{
	...
	SDL_Window* window = SDL_CreateWindow("Coord3DM Windows", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 700, 500, SDL_WINDOW_OPENGL);
	...
	while (STATE != GAMESTATE::EXIT)
	{
		...
		for (int i = 0; i < sizeof(obj_pos) / sizeof(glm::vec3); i++)
		{
			...
			//rotate
			model = glm::rotate(model, (float)SDL_GetTicks() / 1000.0f, glm::vec3(0.5, 1.0, 0.0));
			//scale
