...
void SDLCameraOrb()
{
	...
	SDL_Window* window = SDL_CreateWindow("CameraOrb Windows", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 700, 500, SDL_WINDOW_OPENGL);
	...
	while (STATE != GAMESTATE::EXIT)
	{
		...
		for (int i = 0; i < sizeof(obj_pos) / sizeof(glm::vec3); i++)
		{
			...
			//rotate
			model = glm::rotate(model, (float)SDL_GetTicks() * 3 / 1000.0f, glm::vec3(0.5, 1.0, 0.0));
			//scale
			...
			glm::mat4 view;
			GLfloat camX = sin(SDL_GetTicks() / 1000.0f) * 50.0f;
			GLfloat camY = sin(SDL_GetTicks() / 1000.0f) * 6.0f;
			GLfloat camZ = cos(SDL_GetTicks() / 1000.0f) * 50.0f;
			view = glm::lookAt(glm::vec3(camX, camY, camZ), glm::vec3(0.0, camY, 0.0), glm::vec3(0.0, 1.0, 0.0));
